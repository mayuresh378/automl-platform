...
import { Search, Bell, Sparkles, LogIn, X, Clock, AlertCircle, CheckCircle2, Info, AlertTriangle, Moon, Sun } from 'lucide-react';
      ...
      <div className="ml-auto flex items-center gap-1">
        ...
        <div className="relative">
          <motion.button
            ...
            className={cn(
              'flex items-center justify-center h-7 w-7 rounded-full text-[10px] font-semibold ml-1 cursor-pointer select-none',
              isLoggedIn ? 'bg-primary/20 text-primary' : 'bg-white/10 text-zinc-400',
            )}
          >
            {isLoggedIn ? initials : <LogIn className="h-3.5 w-3.5" />}
          </motion.button>
          <AnimatePresence>
            {profileOpen && (
              <motion.div
              ...
              >
                {isLoggedIn ? (
                  <>
                    ...
                    <div className="px-2.5 py-1.5 text-zinc-400 rounded hover:bg-sidebar-hover cursor-pointer" onClick={() => setActivePage('Settings')}>Profile settings</div>
                    <div className="px-2.5 py-1.5 text-zinc-400 rounded hover:bg-sidebar-hover cursor-pointer" onClick={() => { setSettingsTab('billing'); setActivePage('Settings'); }}>Billing</div>
                    <div className="px-2.5 py-1.5 text-danger rounded hover:bg-sidebar-hover cursor-pointer" onClick={() => { logout(); setProfileOpen(false); }}>Sign out</div>
                  </>
                ) : (
                  <>
                    <div className="px-2.5 py-1.5 text-zinc-300 font-medium">Guest</div>
                    <div className="my-1 border-t border-border" />
                    <div className="px-2.5 py-1.5 text-primary rounded hover:bg-sidebar-hover cursor-pointer flex items-center gap-2" onClick={() => { setSettingsTab('authentication'); setActivePage('Settings'); setProfileOpen(false); }}>
